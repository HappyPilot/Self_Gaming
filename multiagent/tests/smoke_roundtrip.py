...
class SmokeTest:
    ...
    def seed_recorder(self):
        samples = [
            (0.25, ["menu"], "press_enter"),
            (0.35, ["dialog"], "press_enter"),
            (0.45, ["button"], "move_cursor"),
            (0.55, ["field"], "move_cursor"),
        ]
        for idx, (mean, text, action) in enumerate(samples):
            scene_payload = {
                "ok": True,
                "event": "scene_update",
                "mean": mean,
            ...
            time.sleep(0.05)
            action_payload = {"ok": True, "action": action, "source": "smoke_test", "step": idx}
            self.client.publish(self.cfg.act_topic, json.dumps(action_payload), qos=0)
            time.sleep(0.05)
