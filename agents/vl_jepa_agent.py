...
class VlJepaAgent:
    ...
    def _ensure_encoder(self):
        ...
        with self.encoder_lock:
            ...
            if BACKEND == "torchscript":
                ...
                try:
                    ...
                    if str(device).lower().startswith("cuda"):
                        cpu_device = "cpu"
                        try:
                            logger.warning("Retrying SigLIP init on CPU after CUDA failure.")
                            encoder = SiglipEncoder(
                                SIGLIP_MODEL_ID,
                                cpu_device,
                                INPUT_SIZE,
                            ...
                            logger.error("SigLIP CPU fallback init failed: %s", cpu_exc)
            ...
            if getattr(self.encoder, "backend", "") in {"siglip", "siglip2"}:
                logger.info(
