...
def fetch_control_profile(game_hint: str, texts: Optional[list] = None) -> Tuple[Optional[dict], str]:
    ...
    prompt = (
        ...
        "       \"action\": str,\n"
        "       \"keys\": [str, ...],\n"
        "       \"category\": \"movement\"|\"combat\"|\"interaction\"|\"ui\"|\"social\"|\"system\",\n"
        "       \"purpose\": str,\n"
        "       \"contexts\": [str, ...],\n"
        ...
        "  \"confidence\": 0.0-1.0\n"
        "}\n"
        "Rules: be conservative; avoid system keys; keep movement/combat keys minimal and game-safe.\n"
        "Only include WASD if the game is known to use WASD by default. For click-to-move ARPGs, prefer skill keys QWER + 1-5.\n"
        "Put inventory/map/menu/chat keys in category=ui or social (not in gameplay categories).\n"
        "Combos must be explicit (e.g. \"shift+1\").\n"
